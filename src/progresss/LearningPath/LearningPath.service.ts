...
export class LearningPathService {
    ...
    async getLearningPaths(userId: string) {
        ...
        return careerProgress.map((cp) => {
            const careerDetail = careersData.find(c => c.career_id === cp.career_id) || {};
            return {
                id: cp.career_id,
    ...
    async getCareerCourses(userId: string, careerId: number) {
        ...
        const { data: careerProgress, error: careerError } = await this.supabaseService.client
            .from('user_progress_career')
        ...
        const mergedCourses = coursesProgress.map(cp => {
            const courseDetail = coursesData.find(c => c.course_id === cp.course_id);
            return {
    ...
    async getCourseLessons(userId: string, courseId: number) {
        ...
        if (courseError || !courseProgress) {
            throw new NotFoundException('Course progress not found. Please start the career first.');
        }
        ...
        const mergedLessons = lessonsProgress.map(lp => {
            const lessonDetail = lessonsData.find(l => l.lesson_id === lp.lesson_id);
            return {
    ...
    async completeLesson(userId: string, lessonId: number, done: boolean) {
        ...
        const { data: lessonProgress, error: findError } = await this.supabaseService.client
            .from('user_progress_lesson')
            .select('*')
            .eq('user_id', userId)
