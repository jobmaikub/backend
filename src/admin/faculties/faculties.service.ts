...
export class FacultiesService {
  ...
  async createFaculty(data: { eng_name: string; th_name?: string }) {
    ...
    const nextFacultyId = Array.isArray(rpcData)
      ? rpcData[0]?.next_id
      : rpcData?.next_id;
  ...
  async updateFaculty(
    ...
    if (error || !result) {
      throw new NotFoundException(error?.message || 'Faculty not found');
    }
